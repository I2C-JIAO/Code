...
#include "OLED.h"
#include "AD.h"
////这是STM32的第20个程序：本函数使用ADC扫描模式+DMA配合实现完全自动化硬件传输模数转换的数据
//本程序用来同时测量多个模块的电压：光敏、热敏、红外、电位器的电压
uint16_t AD0, AD1, AD2, AD3;//变量用于接收AD转换返回值
...
int main(void)
{
	...
	while(1)
		{
			/*调用DMA存储器数组，依次将AD_Value[0]、[1]、[2]、[3]的值传给用于
			接收AD转换返回值的变量AD0, AD1, AD2, AD3（分别代表电位器、光敏、热敏、红外）*/
			AD0 = AD_Value[0];
			AD1 = AD_Value[1];
			AD2 = AD_Value[2];
			AD3 = AD_Value[3];
