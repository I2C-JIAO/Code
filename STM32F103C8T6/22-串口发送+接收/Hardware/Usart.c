...
//定义一个Serial_RxData变量用来存放串口接收的数据
uint8_t Serial_RxData;
//定义一个Serial_RxFlag变量用来存放串口接收的标志位
...
{
	//调用库函数USART_GetITStatus获取USART1接收的中断标志位
	//判断接收中断标志位是否为1
	if(USART_GetITStatus(USART1, USART_IT_RXNE) == SET)
	{
		//调用库函数USART_ReceiveData接收USART1的数据传给定义的变量Serial_RxData
		Serial_RxData = USART_ReceiveData(USART1);
		//置串口接收的标志位变量Serial_RxFlag为1
		Serial_RxFlag = 1;
		//清除标志位
